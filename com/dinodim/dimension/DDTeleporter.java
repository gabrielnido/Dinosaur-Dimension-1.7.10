...
public class DDTeleporter extends Teleporter 
{
	...
	public DDTeleporter(WorldServer worldServer) 
	{
		super(worldServer);
	...
	@Override
	public void placeInPortal(Entity entity, double par2, double par3, double par4, float par5)
	{
		if(this.worldServerInstance.provider.dimensionId != 1)
		{
			if(!this.placeInExistingPortal(entity, par2, par3, par4, par5))
			{
				this.makePortal(entity);
				this.placeInExistingPortal(entity, par2, par3, par4, par5);
			}
		}
		else
		{
			...
			for (int l = -2; l <= 2; ++l)
			{
				for (int i1 = -2; i1 <= 2; ++i1)
				{
					for (int j1 = -1; j1 < 3; ++j1)
					{
						...
						boolean flag = j1 < 0;
						this.worldServerInstance.setBlock(k1, l1, i2, flag ? DDBlocks.caveBlock : Blocks.air);
					}
				}
			}

			entity.setLocationAndAngles((double)posX, (double)posY, (double)posZ, entity.rotationYaw, 0.0F);
    ...
    public boolean placeInExistingPortal(Entity entity, double d, double d1, double d2, float f)
    {
        ...
        else
        {
            for (l3 = l - short1; l3 <= l + short1; ++l3)
            {
                ...
                for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1)
                {
                    ...
                    {
                        if (this.worldServerInstance.getBlock(l3, i2, l1) == DDBlocks.dinoPortal)
                        {
                            while (this.worldServerInstance.getBlock(l3, i2 - 1, l1) == DDBlocks.dinoPortal)
                            {
                                --i2;
                            }
        ...
        if (d3 >= 0.0D)
        {
            ...
            if (this.worldServerInstance.getBlock(i - 1, j, k) == DDBlocks.dinoPortal)
            {
                i4 = 2;
            }
            ...
            if (this.worldServerInstance.getBlock(i, j, k - 1) == DDBlocks.dinoPortal)
            {
                i4 = 3;
            }
            ...
            for (i3 = -1; i3 <= 1; ++i3)
            {
                for (j3 = 1; j3 < 3; ++j3)
                {
                    for (k3 = -1; k3 < 3; ++k3)
                    {
                        ...
                        j4 = k2 + (j3 - 1) * l2 - i3 * l5;
                        flag = k3 < 0;
                        this.worldServerInstance.setBlock(l3, i4, j4, flag ? DDBlocks.caveBlock : Blocks.air);
                    }
                }
            }
        }
        ...
        for (i3 = 0; i3 < 4; ++i3)
        {
            for (j3 = 0; j3 < 4; ++j3)
            {
                for (k3 = -1; k3 < 4; ++k3)
                {
                    ...
                    this.worldServerInstance.setBlock(l3, i4, j4, (Block)(flag ? DDBlocks.caveBlock : DDBlocks.dinoPortal), 0, 2);
                }
            }
