...
{	
	...
	protected BlockDDPortal() 
	{
		super(Material.portal);
		this.setHardness(-1F);
		this.setBlockName("portal");
		this.setBlockTextureName(RefStrings.MOD_ID + ":portal");
	}
    ...
    public void onBlockAdded(World worldIn, int x, int y, int z) 
    {
    	...
    	if(!DDUtils.isTouchingBlock(worldIn, x, y, z, this) && !this.tryAddPortalBlocks(worldIn, x, y, z))
    	{
	...
	public void onEntityCollidedWithBlock(World worldIn, int x, int y, int z, Entity entity)
	{
		if (entity.ridingEntity == null && entity.riddenByEntity == null && (entity instanceof EntityPlayerMP))
		{
			...
			int dimID = DDDimensionRegistry.dimensionID;
			...
			{
				thePlayer.timeUntilPortal = 10;
				mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, dimID, new DDTeleporter(mcServer.worldServerForDimension(dimID)));
			}
			else if(thePlayer.dimension == dimID)
			{
				thePlayer.timeUntilPortal = 10;
				mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, 0, new DDTeleporter(mcServer.worldServerForDimension(0)));
			}
		}
	}
	...
	public boolean tryAddPortalBlocks(World worldIn, int x, int y, int z)
	{
		if(this.isFrameValid(worldIn, x, y, z))
		{
			...
			for(int i = 0, j = portalLocs.length; i < j; ++i)
			{
				if(isXAligned)
				{
					DDUtils.setBlockIfAir(worldIn, x + offsetXZ + portalLocs[i][0], y + offsetY + portalLocs[i][1], z, this);
				}
				else
				{
					DDUtils.setBlockIfAir(worldIn, x, y + offsetY + portalLocs[i][1], z + offsetXZ + portalLocs[i][0], this);
				}
			}
			return true;
		}
